...
- Asset names (from GoReleaser): `copilot-oauth-proxy_{{.Version}}_{{.Os}}_{{.Arch}}.tar.gz`.
...
6) Verify assets in the Draft release:
   - Assets should be `copilot-oauth-proxy_X.Y.Z_<os>_<arch>.tar.gz` plus `checksums.txt`.
...
- Wrong asset name: Ensure `.goreleaser.yml` `archives.name_template` is `copilot-oauth-proxy_{{ .Version }}_{{ .Os }}_{{ .Arch }}`.
